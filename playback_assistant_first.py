...
async def play_music_core(client, chat_id, song_info, status_msg=None, retry_attempt=False):
    try:
        # Join the configured assistant before attempting playback.
        # This handles assistant membership before download/playback.
        if not retry_attempt:
            joined = await _try_join_assistant(client, chat_id, status_msg)
            if not joined:
                ast_mention = (
                    f"@{state.ASSISTANT_USERNAME}"
                    if state.ASSISTANT_USERNAME
                    else "the assistant"
                )
                verify_btn = InlineKeyboardMarkup([[
                    InlineKeyboardButton(
                        "✅ I added the assistant — retry",
                        callback_data="verify_assistant"
                    )
                ]])
                if status_msg:
                    await status_msg.edit_text(
                        f"❌ <b>Assistant join failed.</b>\n\n"
                        f"Make sure {ast_mention} can join this group, "
                        f"then tap the retry button.",
